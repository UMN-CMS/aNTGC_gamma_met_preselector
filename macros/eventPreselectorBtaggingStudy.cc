...
#include "eventPreselectorBtaggingStudy.h"
#include "TSystem.h"
...
aNTGCpreselector::aNTGCpreselector(std::string _file_list, std::string _output_file){
	...
	if(isQCD)std::cout<<"QCD sample detected!"<<std::endl;
	if(isGJets)std::cout<<"GJets sample detected!"<<std::endl;
...
void aNTGCpreselector::setOutputTree(){
	...
	if(isMC){
		...
		outTree->Branch("genBBarMass", &genBBarMass_);
	}
	...
	outTree->Branch("isGenBtagged", &isGenBtagged_);
...
void aNTGCpreselector::copyEvent(){
	...
	if(isMC){
		...
		/////////////////////////////////////////////// Get gen b tag ////////////////////////////////////////////////
		genBBarMass_ = -9999999.;
		Float_t _zDisplacement_ = 9999999.;
		Float_t _qqEta = 9999999., _qqPhi =9999999.;
		Bool_t _jetHasB  = 0;
		for(UInt_t i = 0; i < _mcPID.size(); i++){
			if(std::abs((_mcPID[i])) != 5) continue;
			if(_mcPt[i] < 10.) continue;

			if(deltaR(_mcEta[i], _mcPhi[i], selectedAk8JetGenEta_, selectedAk8JetGenPhi_) > 0.8) continue;

			 _jetHasB = 1;

			// if((std::abs((_mcPID[i])) == 5) && (_mcPt[i] > 20.) && getBit((_mcStatusFlag[i]), 3)){
			// 	Float_t _deltaR_between_b_ak8 = deltaR((_mcEta[i]), (_mcPhi[i]), (AK8PuppiJet_Eta_), (AK8PuppiJet_Phi_));
			// 	if(_deltaR_between_b_ak8 < 0.4) nBquarksInJet_++;
			// }

			TLorentzVector b1;
			b1.SetPtEtaPhiE ((_mcPt[i]), (_mcEta[i]), (_mcPhi[i]), (_mcE[i]));
			for(UInt_t j = i+1; j < _mcPID.size(); j++){
				if(std::abs((_mcPID[j])) != 5) continue;
				if(_mcPt[j] < 10.) continue;
				if(deltaR(_mcEta[j], _mcPhi[j], selectedAk8JetGenEta_, selectedAk8JetGenPhi_) > 0.8) continue;
				TLorentzVector b2;
				b2.SetPtEtaPhiE ((_mcPt[j]), (_mcEta[j]), (_mcPhi[j]), (_mcE[j]));

				TLorentzVector sumbb = b1 + b2;

				Float_t tmp_zDisplacement_ = std::abs(sumbb.M() - 91.2);

				if(tmp_zDisplacement_ < _zDisplacement_) {
					_zDisplacement_ = tmp_zDisplacement_;
					_qqEta = sumbb.Eta();
					_qqPhi = sumbb.Phi();
					genBBarMass_ = sumbb.M();
				}
			}
		};

		if(AK8PuppiJet_PartonFlavour_ == 5) setBit(isGenBtagged_, 0, 1);
		if(AK8PuppiJet_HadronFlavour_ == 5) setBit(isGenBtagged_, 1, 1);
		if(_zDisplacement_ < 40. && deltaR(_qqEta, _qqPhi, selectedAk8JetGenEta_, selectedAk8JetGenPhi_) < 0.8) setBit(isGenBtagged_, 2, 1);
		if(_jetHasB) setBit(isGenBtagged_, 3, 1);

		p_genBtag = (Float_t) isGenBtagged_ + 0.001;
	}
};
...
//	registerCutFlow();
	...
	for(Int_t i = 0; i < _nAK8PuppiJets; i++){
		...
		if(std::abs((_AK8PuppiJet_CHSPrunedMass[i]) - ZMASS) > 50.) continue;
