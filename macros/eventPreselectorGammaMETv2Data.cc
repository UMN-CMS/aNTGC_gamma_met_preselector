...
aNTGCpreselector::aNTGCpreselector(std::string _file_list, std::string _output_file, std::string _mcPUfile, std::string _dataPUfile, Float_t _xSec=-1.){
	...
	"IsMC: "<<isMC<<std::endl<<
	"Is 2016 MC: "<<is2016MCsample<<std::endl;
...
Bool_t aNTGCpreselector::selectGammaMETevent(){
	...
	if(!getBit(tmp_HLTPho, 9) && !is2016MCsample) return 0;// 9 = HLT_Photon200_v, 11 = HLT_Photon300_NoHE_v, 19 = HLT_Photon135_PFMET100_v, no trigger on 2016 MC
	registerCutFlow();
...
void aNTGCpreselector::analyze(){
	...
	while (inputTTreeReader.Next()) {
		...
		if(isMC){
			...
			eventWeight_ = (_genWeight) * puWeight_ * xSec;
